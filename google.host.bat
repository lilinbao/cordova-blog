...
%EchoAndLog% downloading from GitHub file name hosts...
...
%EchoAndLog% downloaded.
echo.

if exist %HostsPath% (
    call :backuphosts
) else (
    %EchoAndLog% The origin host file doesn't exist - skip backup host file
)
%EchoAndLog% replacing hosts file...
move %DLPath% %HostsPath% %LogToFile% || call :error hosts 文件替换失败.
%EchoAndLog% hosts file replacement completed.
echo.

%EchoAndLog% refreshing DNS cache...
ipconfig /flushdns %LogToFile% || call :error DNS refresh failed.
%EchoAndLog% DNS refresh successful.
echo.
%EchoAndLog% [All task completed!]
echo.

echo press any key to access google.com.hk click close button to quite
pause >nul
start https://www.google.com.hk
echo google.com.hk opened, please check
echo.
goto :end

:backuphosts
%EchoAndLog% backup hosts file...
set "bakfilename=hosts_%date%_%time:~0,8%.bak"
set bakfilename=%bakfilename:/=-%
set bakfilename=%bakfilename:\=-%
set bakfilename=%bakfilename::=-%
set bakfilename=%bakfilename: =_%
copy %HostsPath% %BackupDir%\%bakfilename% %LogToFile% || call :error hosts backup file failed.
%EchoAndLog% original hosts has been backup to %BackupDir%\%bakfilename%.
echo.
goto :eof
...
%EchoAndLog% Error：%*
start %LogFilePath%
echo error log file has been open
goto :end
